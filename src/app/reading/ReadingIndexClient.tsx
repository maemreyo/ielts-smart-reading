...
// Type augmentation for storage API
declare global {
  interface Window {
    storage: {
...
class PassageStorage {
  ...
  static scheduleSave(data: StorageData) {
    ...
    this.saveTimeout = setTimeout(async () => {
      if (this.pendingData) {
        try {
          await window.storage.set(this.STORAGE_KEY, JSON.stringify(this.pendingData));
  ...
  static async saveNow(data: StorageData) {
    if (typeof window === 'undefined' || !window.storage) return;
    ...
    try {
      await window.storage.set(this.STORAGE_KEY, JSON.stringify(data));
  ...
  static async toggleBookmark(passageId: string, currentData: StorageData): Promise<StorageData> {
